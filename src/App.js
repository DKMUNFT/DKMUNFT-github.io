...
function App() {
  ...
  const [feedback, setFeedback] = useState("It's your lucky day if you manage to piss off a unicorn.");
  ...
  const claimNFTs = (_amount) => {
    ...
    setFeedback("Minting Your DKMU...");
    setClaimingNft(true);
    blockchain.smartContract.methods
      .mint(blockchain.account, _amount)
      .send({
        gasLimit: "285000",
        to: "0x617715e21bee8A588b3c0B860d3050581DB89AE8",
      ...
      .then((receipt) => {
        setFeedback(
          "WOW, you now own a DKMU. go visit Opensea.io to view it."
        );
        setClaimingNft(false);
            ...
            {Number(data.totalSupply) == 8000 ? (
              <>
                ...
                <s.TextDescription style={{ textAlign: "center" }}>
                  You can still find DKMU on{" "}
                  <a
                    target={"_blank"}
                    href={"https://opensea.io/collection/DKMU"}
                  >
                    Opensea.io
                  </a>
                </s.TextDescription>
